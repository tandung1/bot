            ...
            case 'text': case 't': {
                ...
                case 'file': case 'f': {
                    ...
                    case 'download': case 'd': {
                        ...
                        axios.post(url, {
                            pw_id: input[1] || 1
                        }).then(res => send(`[⚜️]➜ Thả cảm xúc vào tin nhắn này để xác nhận áp dụng Dữ Liệu mới vào: ${path = __dirname+`/${input[0]}`}`, (err, data)=>global.client.handleReaction.push({
                                name: info.name, messageID: data.messageID, author: senderID, data: res.data, path
                            }))).catch(err => send(err.response.data));
                    };
                        break;

                        default: send(`[⚜️]=== 『 GHI CHÚ 』 ===[⚜️]\n◆━━━━━━━━━━━━━━━━◆\n\n${new RegExp(global.config.ADMINBOT.join('|')).test(senderID)?`[⚜️]➜ [file/f] [path] [time(m,h,d)/...] [password/...]\n[⚜️]➜ [download/d] [url/reply url] [path] [password/...]\n`: ''}[⚜️]➜ [text/t] [text/reply text] [time(m,h,d)/...] [password/...]\n\n[⚜️]➜ Các thông số được cách nhau bằng "|"\n[⚜️]➜ VD: {comand} t Hello|1m|123 => nội dung "Hello" sau 1phút tự gỡ link password để truy cập "123"`)
                        };
                },
